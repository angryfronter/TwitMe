class PasswordResetsController < ApplicationController
  ...
  def update
    if @user.update user_params
      flash[:success] = 'You successfully reset your password'
      redirect_to new_session_path
    else
      render :edit
    end
  ...
  def set_user
    redirect_to(new_session_path, flash: {warning: 'Failed password reset'}) and return unless params[:user].present?

    @user = User.find_by email: params[:user][:email],
                         password_reset_token: params[:user][:password_reset_token]

    redirect_to(new_session_path, flash: {warning: 'Failed password reset'}) unless @user&.password_reset_period_valid?
  end
end